...
export default function AdminJobsClient(){
  ...
  async function runImport(){
    setLoading(true);setMessage('Running ATS import…');
    const r=await fetch('/api/admin/import-jobs',{method:'POST'});const d=await r.json();setLoading(false);
    setMessage(r.ok?`Import finished: ${d.newCount||0} new · ${d.updatedCount||0} updated · ${d.deactivatedCount||0} deactivated · ${d.successCount||0}/${d.sourceCount||0} sources successful${d.failureCount?` · ${d.failureCount} failed`:''}`:d.error||'Import failed');
    if(r.ok)load();
  }
